...
public class Tour {
	...
	public boolean addStopSlice(Location location, ArrayList<Location> locationsSlice,
	...
	public boolean addNextStopPizza(ArrayList<Location> locations) {
		if (!locations.isEmpty()) {
			...
			if (!Main.isUsed() && tourStops.size() == 1) {
				...
				// winkel
				// neu!
				Main.setUsed(true); // if true: benutzt die beiden ifups nur ein
				// einziges mal
		...
		if (Main.getLastLocation() == null) {
			...
			if (tourStops.size() == 1) {
				if (locations.isEmpty()) {
					...
					return addStop(Main.findClosestLocation(Main.findClosestLocation(Main.getLastLocation(), locations),
							locations), locations);
				}
			} else {
				if (locations.isEmpty()) {
	...
	public boolean addNextStopSlices(ArrayList<Location> locations) {
		...
		if (!slice1.isEmpty()) {
			return addStopSlice(Main.findClosestLocation(tourStops.get(tourStops.size() - 1), slice1), slice1,
					locations);
		} else {
			if (!slice2.isEmpty()) {
				return addStopSlice(Main.findClosestLocation(tourStops.get(tourStops.size() - 1), slice2), slice2,
						locations);
			} else {
				if (!slice3.isEmpty()) {
					return addStopSlice(Main.findClosestLocation(tourStops.get(tourStops.size() - 1), slice3), slice3,
							locations);
				} else {
					if (!slice4.isEmpty()) {
						return addStopSlice(Main.findClosestLocation(tourStops.get(tourStops.size() - 1), slice4),
								slice4, locations);
					} else {
						if (!slice5.isEmpty()) {
							return addStopSlice(Main.findClosestLocation(tourStops.get(tourStops.size() - 1), slice5),
									slice5, locations);
						} else {
							if (!slice6.isEmpty()) {
								return addStopSlice(
								...
								if (!slice7.isEmpty()) {
									...
									if (!slice8.isEmpty()) {
										...
										if (!slice9.isEmpty()) {
											return addStopSlice(Main.findClosestLocation(
